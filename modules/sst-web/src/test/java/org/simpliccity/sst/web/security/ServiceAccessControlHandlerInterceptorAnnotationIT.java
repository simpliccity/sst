...
@WebAppConfiguration
@ContextConfiguration(classes=ServiceAccessControlHandlerInterceptorConfig.class)
@ActiveProfiles("serviceAccessControl")
public class ServiceAccessControlHandlerInterceptorAnnotationIT 
{
	@Autowired
	...
	@Test
	public void testUriPatternAccessDeniedRole() throws Exception
	{
		String jwtToken = generateJwtToken("user3", "ROLE_UNTRUSTED");

		this.client.perform(get("/greeting/3").header(XAuthHeaderTokenHandler.HEADER_NAME, jwtToken))
			.andExpect(status().isUnauthorized());
	}
