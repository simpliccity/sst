...
@ServiceAccessValidator("userNumber")
public class UserNumberServiceAccessControlRule implements ServiceAccessControlRule 
{

	@Override
	public boolean allowAccess(Principal user, Object content) 
	{
		String userDigit = user.getName().substring(user.getName().length() - 1);

		String greetingId = (content == null) ? "" : content.toString();
		
		return greetingId.equals(userDigit);
	}
}
