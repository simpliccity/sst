...
public class EnvironmentConfigurationDispatcherServlet extends DispatcherServlet 
{
	...
	private EnvironmentConfigurer getConfigurerInstance()
	{
		...
		ClassLoader loader = config.getServletContext().getClassLoader();
		
		Class<?> configurerClass = null;
		try 
		{
			logger.debug("Attempting to load configurer class [" + parameter + "].");
			configurerClass = ClassUtils.forName(parameter, loader);
		} 
